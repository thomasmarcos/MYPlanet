...
def main(context):
	if bpy.data.objects.get("Croute") is None:
	    ...
	    mat = bpy.data.materials.get("s_mercuryreal")
	    if mat is None:
	        mat = bpy.data.materials.append(name="s_mercuryreal")
...
class croute_mercuryreal(bpy.types.Operator):
	"""Tooltip"""
	bl_idname = "myops.croute_mercuryreal"
	bl_label = "s_mercuryreal"
...
def register():
	bpy.utils.register_class(croute_mercuryreal)

def unregister():
	bpy.utils.unregister_class(croute_mercuryreal)

if __name__ == "__main__":
	register()

	# test call
	# bpy.ops.myops.croute_mercuryreal()