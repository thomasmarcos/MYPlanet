...
def main(context):
	if bpy.data.objects.get("Croute") is None:
	    ...
	    mat = bpy.data.materials.get("s_jupiter")
	    if mat is None:
	        mat = bpy.data.materials.append(name="s_jupiter")
...
class croute_jupiter(bpy.types.Operator):
	"""Tooltip"""
	bl_idname = "myops.croute_jupiter"
	bl_label = "s_jupiter"
...
def register():
	bpy.utils.register_class(croute_jupiter)

def unregister():
	bpy.utils.unregister_class(croute_jupiter)

if __name__ == "__main__":
	register()

	# test call
	# bpy.ops.myops.croute_jupiter()