...
def main(context):
	if bpy.data.objects.get("Croute") is None:
	    ...
	    mat = bpy.data.materials.get("S_Earth")
	    if mat is None:
	        mat = bpy.data.materials.append(name="S_Earth")
...
class croute_earth(bpy.types.Operator):
	"""Tooltip"""
	bl_idname = "myops.croute_earth"
	bl_label = "S_Earth"
...
def register():
	bpy.utils.register_class(croute_earth)

def unregister():
	bpy.utils.unregister_class(croute_earth)

if __name__ == "__main__":
	register()

	# test call
	# bpy.ops.myops.croute_earth()