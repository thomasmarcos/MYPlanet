...
def main(context):
	if bpy.data.objects.get("Croute") is None:
	    ...
	    mat = bpy.data.materials.get("s_sun")
	    if mat is None:
	        mat = bpy.data.materials.append(name="s_sun")
...
class croute_sun(bpy.types.Operator):
	"""Tooltip"""
	bl_idname = "myops.croute_sun"
	bl_label = "s_sun"
...
def register():
	bpy.utils.register_class(croute_sun)

def unregister():
	bpy.utils.unregister_class(croute_sun)

if __name__ == "__main__":
	register()

	# test call
	# bpy.ops.myops.croute_sun()