...
def main(context):
	if bpy.data.objects.get("Croute") is None:
	    ...
	    mat = bpy.data.materials.get("S_Mars")
	    if mat is None:
	        mat = bpy.data.materials.append(name="S_Mars")
...
class croute_mars(bpy.types.Operator):
	"""Tooltip"""
	bl_idname = "myops.croute_mars"
	bl_label = "S_Mars"
...
def register():
	bpy.utils.register_class(croute_mars)

def unregister():
	bpy.utils.unregister_class(croute_mars)

if __name__ == "__main__":
	register()

	# test call
	# bpy.ops.myops.croute_mars()